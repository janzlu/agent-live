...
Monitors Antigravity IDE's active conversation transcript in real-time,
...
def get_ide_chat_snapshot(use_cache: bool = True) -> Dict[str, Any]:
    """
    解析当前 IDE 对话状态的即时快照。
...
class ActionBatcher:
    ...
    async def flush(self):
        ...
        narration = ""
        has_modify = any(t in ("replace_file_content", "multi_replace_file_content", "write_to_file") for t in tool_names)
        has_test = any("test" in d.lower() for d in descs) or any(t == "run_command" and "test" in d.lower() for t, d in actions)
        has_git = any("git" in d.lower() for d in descs)
        has_view = any(t in ("view_file", "grep_search", "list_dir") for t in tool_names)
